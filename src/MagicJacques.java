...
public class MagicJacques extends CardGame {
    ...
    public boolean playStageTwo() {
        ...
        String nextInp = lineOut(
                ...
                new String[] { "Wear the clown's shoes - \"wear the shoes\"" });
        while (!nextInp.equals("wear the shoes")) {
        ...
        while (!nextInp.equalsIgnoreCase("kind of scary")) {
            while (!(nextInp.equalsIgnoreCase("very scary") || nextInp.equalsIgnoreCase("kind of scary")
                    || nextInp.indexOf("not scary") >= 0)) {
                nextInp = lineOut(
            ...
            if (nextInp.equalsIgnoreCase("very scary")) {
                nextInp = lineOut(
                        ...
                        new String[] { "Pick the very scary door - \"very scary\"",
                                "Pick the kind of scary door - \"kind of scary\"",
                                "Pick the not scary door - \"not scary\"" });
            } else if (nextInp.equalsIgnoreCase("not scary")) {
                System.out.println(
        ...
        lineOut("\nYou step into a dark room. As your eyes adjust, you hear a voice come through a speaker...\n\nAND UP NEXT, WE HAVE THE INCREDIBLE MAGIC JACQUES! You can faintly hear people cheering.",
                "Your eyes adjusted. Hit enter.", new String[] { "Do nothing - [enter]" });
        nextInp = lineOut(
        ...
        lineOut("You nod your head, completley bewildered. The rabbit begins:\n\"Hand me your card deck.\" You quitely obey. The rabbit quickly forms two piles of 13 cards, removing the jokers. He hands one pile back to you.",
                "Do nothing.", new String[] { "Do nothing - [enter]" });
                */
        lineOut("\"Well, Jacques. Let's play some slap jack.\"", "You don't have an option anymore. Hit enter!",
                new String[] { "Start Game - [enter]" });
        SlapJack s = new SlapJack(26, "Rabbit");
        int rounds = 1;
        while (!s.playGame() && rounds < 3) {
            rounds++;
    ...
    public void playStageThree() {
        lineOut("(you passed stage 2. One stage left! Hit enter to continue...)", "You're asking for a hint now? You just passed the stage. Just hit enter.",
                new String[] { "Do nothing - [enter]" });
        lineOut("Just as you are ready to celebrate your hard-earned victory against the rabbit, a short, plump man swings open a door. \"JACQUES? WHAT ARE YOU DOING? GET ON STAGE!\"", "You better go on stage.", new String[] {"Get on stage - [enter]"});
        lineOut("The rabbit jumps on top of your head as the plump man runs over and places the top hat over the rabbit.\n\"Goodness, are those Michael's shoes? Where is that clown?!\" You're not sure whether to tell the man what happened.", "Idk man, go with the flow.", new String[] {"Do nothing - enter"});
        lineOut("As you walk to the stage, you hear the audiences cries grow lowder. Your heart thumps as the thought of performing. You try to remember some magic trick - any magic trick - that you can perform, but your mind draws a blank.", "Wait for it...", new String[] {"Do nothing - enter"});
        lineOut("The roar of the audience echoes in your chest as you move closer to the stage. You begin wishing that clown ended it for you before you got here...", "No hint available", new String[] {"Do nothing - enter"});
        lineOut("You see the light of the stage as you pause momentairly on the side. You take a deep breath, clutch your playing cards, and close your eyes...", "No hint available", new String[] {"Do nothing - enter"});
        lineOut("\"Jacques! Jacques! Jacques!\" The audience screams.", "No hint available", new String[] {"Do nothing - enter"});
        ...
        lineOut("\"What on earth are you doing? Did you sleep here last night?\"", "No hint available", new String[] {"Do nothing - enter"});
        lineOut("You look up. You see the green vines of the Airbnb office and realize...you're at work! You look back and see your manager with the face of someone who's seen a ghost.", "No hint available", new String[] {"Do nothing - enter"});
        lineOut("You look at your laptop, covered in drool (gross). It's 10AM ... ", "No hint available", new String[] {"Do nothing - enter"});
        lineOut("After apologizing profously to your manager, you get up and walk back to your desk.", "No hint available", new String[] {"Do nothing - enter"});
        lineOut("You sit down, deeply breathing to remind yourself it was all a dream.", "No hint available", new String[] {"Do nothing - enter"});
        lineOut("Just as you calm down, you look to your co-workers desk and see ... a rabbit playing with cards??!!", "No hint available", new String[] {"Do nothing - enter"});
