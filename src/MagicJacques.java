...
public class MagicJacques extends CardGame {
  ...
  public void playStageOne() {
    ...
    int invalidTries = 0;
    while (!isValidCard(cardPickedUp) && invalidTries <= 3) {
      if (invalidTries == 3) {
        System.out.println(
  ...
  public void userWon(boolean inCostume) {
    if (inCostume) {
      ...
      lineOut(
        ...
        lineOut("You try to say something to explain! Before you can respond, the other Jacques pulls out a Joker. You know what happens next...", "No hint available.", new String[] {"Do nothing - [enter]"});
